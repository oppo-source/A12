...
// be careful, if avc: denied { dac_override } for pid=41 comm="opmonitor_boot" capability=1
//     scontext=u:r:kernel:s0 tcontext=u:r:kernel:s0 tclass=capability , try creds_change_dac
    creds_change_id();
#else
    creds_change_dac();
#endif
    ...
    if (IS_ERR(devfile))
    {
        pr_err("read open /proc/devinfo/emmc error: (%ld)\n",PTR_ERR(devfile));
        devfile = filp_open("/proc/devinfo/ufs", O_RDONLY, 0440);
        if(IS_ERR(devfile))
        {
            pr_err("read open /proc/devinfo/ufs error: (%ld)\n",PTR_ERR(devfile));
            return -1;
        } else {
            ufs_type = 1;
...
//     scontext=u:r:kernel:s0 tcontext=u:r:kernel:s0 tclass=capability , try creds_change_dac
    ...
    if (IS_ERR(devfile))
    {
        pr_err("write open /proc/devinfo/emmc error: (%ld)\n",PTR_ERR(devfile));
        devfile = filp_open("/proc/devinfo/ufs", O_RDONLY, 0440);
        if(IS_ERR(devfile))
        {
            pr_err("open /proc/devinfo/ufs error: (%ld)\n",PTR_ERR(devfile));
            return -1;
        } else {
            ufs_type = 1;
            offsize = OPPO_UFS_PARTITION_BOOTLOG_OFFSET + OPPO_UFS_PARTITION_SPEC_FLAG_OFFSET;
    ...
    if (IS_ERR(opfile)) {
        pr_err("write open OPPO_PARTITION_OPPORESERVE_1 error: %ld\n",PTR_ERR(opfile));
