...
static int hx83102d_0f_overlay(int ovl_type, int mode)
{
	...
	if (IN_SELF_TEST == 0) {
    /*3. check mode need to enter gesture mode or not*/
		if (mode == 0) {
			send_data[3] = 0x00;
			send_data[2] = 0x00;
			send_data[1] = 0x00;
			send_data[0] = 0x00;
			himax_register_write(gest_addr, 4, send_data, false);
		} else {
...
//	        himax_set_SMWP_enable(1, 1);
            ...
            send_data[0] = 0x5A;
            himax_register_write(gest_addr, 4, send_data, false);
/* Machinism for waiting for fw OK-> in diplay >> touch 
*	        count = 0;
*	        do {
...
#ifdef ODM_WT_EDIT
//Longfei.Wang@ODM_WT.BSP.TP,2020/02/18,add for WT hardwareinfo
    if (g_tp_dev_vendor == 1) {
        sprintf(Ctp_name, "HLT,HX83102D,FW:0x%x\n", data[2]);
    }
    TPD_INFO("himax_tddi_ic_get_fw_ver Ctp_name is : %s\n", Ctp_name);
//Longfei.Wang@ODM_WT.BSP.TP,2020/02/24,add for oppo hardwareinfo
...
//Longfei.Wang@ODM_WT.BSP.TP,2020/02/18,add for WT hardwareinfo
    if (g_tp_dev_vendor == 1) {
        sprintf(Ctp_name, "HLT,HX83102D,FW:0x%02x\n", touch_ver>>8);
    }
    TPD_INFO("himax_tddi_ic_get_fw_ver Ctp_name is : %s\n", Ctp_name);
//Longfei.Wang@ODM_WT.BSP.TP,2020/02/24,add for oppo hardwareinfo
...
/*Himax_DB_Test End*/   
    ...
    if (raw_file == NULL)
        raw_file = file_open_name(vts_name, O_TRUNC | O_CREAT | O_RDWR, 0666);
