...
int g_f_0f_updat;
	...
	#ifdef HX_TP_HEADSET_NOTIFIER
	static unsigned long himax_tp_headset_state = 0;
	#endif
#endif
...
int himax_common_proc_init(void)
{
		...
		himax_touch_proc_dir = proc_mkdir(HIMAX_PROC_TOUCH_FOLDER, himax_proc_touchpanel_dir);
...
static ssize_t himax_oppo_tp_direction_write(struct file *file, const char *buff, /*tp_fw_update*/
									size_t len, loff_t *pos)
{
	char buf_tmp[4] = {0};
...
#ifdef HX_SMART_WAKEUP
static ssize_t himax_double_tap_en_read(struct file *file, char *buf, size_t len, loff_t *pos)
{
	struct himax_ts_data *ts = private_ts;
	size_t count = 0;
	char *temp_buf;
...
static ssize_t himax_tp_fw_update_write(struct file *file, const char *buff, size_t len, loff_t *pos)
{
	char buf_tmp[12] = {0};
	if ( *pos ) {
    	E("%s,is already read the file\n",__func__);
...
#ifdef HX_TP_HEADSET_NOTIFIER
static bool himax_headset_mode_set(void)
{
	...
	do {
		if (himax_tp_headset_state == 0x01) {
			himax_in_parse_assign_cmd(fw_func_handshaking_pwd, tmp_data, 4);
			g_core_fp.fp_flash_write_burst(pfw_op->addr_headline, tmp_data);
		...
		g_core_fp.fp_register_read(pfw_op->addr_headline, FOUR_BYTE_DATA_SZ, tmp_data, 0);
...
static int i_get_FW(void)
{
	...
	I("file name = %s\n", BOOT_UPGRADE_FWNAME);
	ret = request_firmware(&image, BOOT_UPGRADE_FWNAME, private_ts->dev);
	if (ret < 0) {
#if defined(__EMBEDDED_FW__)
		image = &g_embedded_fw;
...
#if defined(HX_ZERO_FLASH) && defined(HX_RESUME_SET_FW)
#if defined(HX_SMART_WAKEUP)
	if (ts->SMWP_enable == 0  || private_ts->in_self_test == 1) {
#endif
		private_ts->in_self_test = 0;
		mutex_lock(&private_ts->fw_update_lock);	
