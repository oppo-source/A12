...
#ifdef ODM_WT_EDIT
//Longfei.Wang@ODM_WT.BSP.TP,2020/2/4,add for WT hardwareinfo
...
static void himax_mcu_read_FW_ver(void)
{
    ...
    I("himax_mcu_read_FW_ver Ctp_name is : %s\n", Ctp_name);
#endif
...
int himax_zf_part_info(const struct firmware *fw_entry)
{
	...
	part_num = fw_entry->data[offset + 12];
	I("%s, partition number is %d\n", __func__, part_num);
	if (part_num < 2) {
	...
	memcpy(buf, &fw_entry->data[offset], 16);
	...
	for (i = 1; i < part_num; i++) {
		memcpy(buf, &fw_entry->data[i * 0x10 + offset], 16);
