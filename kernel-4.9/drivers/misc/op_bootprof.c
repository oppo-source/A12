...
#define BOOT_COMPLETE_SIZE 6
#define BOOT_FROM_SIZE 16

extern void phx_monit(const char *monitoring_command);
...
int op_boot_finish = 0;
char boot_complete[BOOT_COMPLETE_SIZE]={'\0'};
char boot_from[BOOT_FROM_SIZE]={'\0'};

#define MSG_SIZE 128

void op_log_boot(char *str)
{
...
static ssize_t
op_bootcomplete_write(struct file *filp, const char *ubuf, size_t cnt, loff_t *data)
{
	size_t copy_size = cnt;

	if (cnt >= sizeof(boot_complete))
		copy_size = BOOT_COMPLETE_SIZE - 1;

	if (copy_from_user(&boot_complete, ubuf, copy_size))
		return -EFAULT;

	boot_complete[copy_size] = 0;
...
static int op_bootcomplete_show(struct seq_file *m, void *v)
{
	SEQ_printf(m, "%s\n", boot_complete);
	return 0;
}
...
static int op_bootcomplete_open(struct inode *inode, struct file *file)
{
	return single_open(file, op_bootcomplete_show, inode->i_private);
}
...
static const struct file_operations op_bootcomplete_fops = {
	.open = op_bootcomplete_open,
	.write = op_bootcomplete_write,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
static int __init init_boot_prof(void)
{
	...
	pe = proc_create("opbootcomplete", 0664, NULL, &op_bootcomplete_fops);
