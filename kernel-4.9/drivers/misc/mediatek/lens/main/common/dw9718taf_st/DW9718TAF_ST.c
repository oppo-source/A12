...
#define AF_DRVNAME "DW9718TAF_ST_DRV"
...
static int initAF(void)
{
	...
	if (*g_pAF_Opened == 1) {
		...
		if (i4RetValue < 0) {
			LOG_INF("I2C send 0x00 0x01 failed!!\n");
		}
		i4RetValue = i2c_master_send(g_pstAF_I2Cclient, puSendCmd1, 2);

		if (i4RetValue < 0) {
			LOG_INF("I2C send 0x00 0x00 failed!!\n");
		}
		mdelay(1);
...
/* ////////////////////////////////////////////////////////////// */
long DW9718TAF_ST_Ioctl(struct file *a_pstFile, unsigned int a_u4Command,
...
int DW9718TAF_ST_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	LOG_INF("Start\n");

	if (*g_pAF_Opened == 2) {
//#ifdef ODM_HQ_EDIT
//zangyufei@ODM_HQ.Camera.Driver.20191115 add for AF_release
		//int i4RetValue = 0;
		//char puSendCmd[2] = {0x00, 0x01};
		while(g_u4CurrPosition > 25)
		{
			g_u4CurrPosition = g_u4CurrPosition >>1;
...
int DW9718TAF_ST_SetI2Cclient(struct i2c_client *pstAF_I2Cclient,
...
int DW9718TAF_ST_GetFileName(unsigned char *pFileName)
{
