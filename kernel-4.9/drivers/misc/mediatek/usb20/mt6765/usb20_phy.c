...
#ifdef ODM_HQ_EDIT
/*Hanxing.Duan@ODM.HQ.BSP.CHG.Basic 2019.01.07 modify otg tuning value*/
#define DEVICE_ACTIVE		1
#define HOSTS_ACTIVE		2
#define DEVIECE_VRT_REF		7
#define DEVIECE_TERM_REF	7
#define DEVIECE_ENHANCE		2
#define HOST_VRT_REF		2
#define HOST_TERM_REF		2
#define HOST_ENHANCE		2
...
void usb_phy_tuning(void)
{
    ...
    if (!mtk_musb->is_host) {
		...
		USBPHY_SET32(OFFSET_RG_USB20_VRT_VREF_SEL,
					DEVIECE_VRT_REF << SHFT_RG_USB20_VRT_VREF_SEL);
		...
		USBPHY_SET32(OFFSET_RG_USB20_TERM_VREF_SEL,
					DEVIECE_TERM_REF << SHFT_RG_USB20_TERM_VREF_SEL);

		USBPHY_CLR32(OFFSET_RG_USB20_PHY_REV6,
					VAL_MAX_WIDTH_2 << SHFT_RG_USB20_PHY_REV6);
		USBPHY_SET32(OFFSET_RG_USB20_PHY_REV6,
					DEVIECE_ENHANCE<<SHFT_RG_USB20_PHY_REV6);
	} else {
		USBPHY_CLR32(OFFSET_RG_USB20_VRT_VREF_SEL,
					VAL_MAX_WIDTH_3 << SHFT_RG_USB20_VRT_VREF_SEL);
		USBPHY_SET32(OFFSET_RG_USB20_VRT_VREF_SEL,
					7 << SHFT_RG_USB20_VRT_VREF_SEL);

		USBPHY_CLR32(OFFSET_RG_USB20_TERM_VREF_SEL,
					VAL_MAX_WIDTH_3 << SHFT_RG_USB20_TERM_VREF_SEL);
		USBPHY_SET32(OFFSET_RG_USB20_TERM_VREF_SEL,
					2 << SHFT_RG_USB20_TERM_VREF_SEL);
	...
	pr_err("mtk_usb_phy_tuning is_host_mode:%d\n", mtk_musb->is_host);
}
#else /*ODM_HQ_EDIT*/
void usb_phy_tuning(void)
{
