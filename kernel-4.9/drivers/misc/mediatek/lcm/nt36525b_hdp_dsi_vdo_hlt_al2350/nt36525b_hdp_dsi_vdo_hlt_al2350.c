...
//	params->dsi.vertical_frontporch_for_low_power = 540;
	...
	params->dsi.ssc_disable = 1; /*SSC config enable*/
...
{
	if (gesture_flag == 1) {
		LCM_LOGI("%s: enter gesture_flag = 1\n", __func__);
		LCM_LOGI("%s: exit gesture_flag = 1\n", __func__);
	} else {
		LCM_LOGI("%s: enter gesture_flag = 0\n", __func__);

		lcm_set_pwr_n(0);
		MDELAY(2);
		lcm_set_pwr(0);

		LCM_LOGI("%s: exit gesture_flag = 0\n", __func__);
	}
}
...
static void lcm_init_lcm(void)
{
	...
	pr_err("NVT-ts: lcd call tp resume %s:\n", __func__);
...
//	LCM_LOGI(" %s,backlight: level = %d\n", __func__, level);
	bl_level[1].para_list[0]=(level&0x7ff)>>8;/*get hight 3bit*/
	bl_level[1].para_list[1]=level&0xff;/*get low 8bit*/
...
struct LCM_DRIVER nt36525b_hdp_dsi_vdo_hlt_al2350_lcm_drv = {
	.name = "nt36525b_hdp_dsi_vdo_hlt_al2350",
