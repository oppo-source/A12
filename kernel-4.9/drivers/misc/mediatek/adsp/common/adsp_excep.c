...
{
	void *mem_addr = (void *)adsp_get_reserve_mem_virt(id);
	size_t mem_size = (size_t)adsp_get_reserve_mem_size(id);
				    ...
				    enum adsp_core_id id)
{
	...
	if (n != sizeof(struct MemoryDump))
		pr_info("%s(), size not match n(%x) != MemoryDump(%zu)",
...
static ssize_t adsp_A_dump_log_show(struct file *filep, struct kobject *kobj,
				struct bin_attribute *attr,
				char *dst, loff_t offset, size_t size)
{
	void *src = (void *)adsp_get_reserve_mem_virt(ADSP_A_LOGGER_MEM_ID);
	size_t length = adsp_get_reserve_mem_size(ADSP_A_LOGGER_MEM_ID);

	if (!src || !dst || offset > length)
		return 0;
