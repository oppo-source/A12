...
static TZ_RESULT KREE_OpenFd(const char *port, int32_t *Fd)
{
	...
	*Fd = tmp;
	...
	KREE_DEBUG("KREE_OpenFd: Fd = %d, chan_p = %p\n", *Fd, h.dn);
...
//#ifdef VENDOR_EDIT
//Dongnan.Wu@BSP.Fingerprint.Basic, 2019/07/12, modify the of_match struct to the right way.
	...
	for (i = 0 ; i < MAX_GZ_MVA_MAP ; i++) {
		if (gz_mva_map_table[i].region_id == region_id) {
			...
			ret = m4u_alloc_mva(m4u_gz_client, M4U_PORT_VPU,
				0, sg, size, M4U_PROT_READ | M4U_PROT_WRITE,
