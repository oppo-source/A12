		...
		do {
			*sensor_id = return_sensor_id() & 0xffef;
			if (*sensor_id == imgsensor_info.sensor_id) {
				#ifdef HARDWARE_INFO
				hardwareinfo_set_prop(HARDWARE_BACK_SUB_CAM, "GC02K0");
				hardwareinfo_set_prop(HARDWARE_BACK_SUB_CAM_MOUDULE_ID, "lianheyingxiang");
				#endif
...
static kal_uint32 streaming_control(kal_bool enable)
{
	...
	if (enable){
		write_cmos_sensor(0xfe, 0x00);
		write_cmos_sensor(0xed, 0x90); /* stream on */
		write_cmos_sensor(0xfe, 0x00);
	} else {
		write_cmos_sensor(0xfe, 0x00);
		write_cmos_sensor(0xed, 0x00); /* stream off */
		write_cmos_sensor(0xfe, 0x00);
	}
	mdelay(2);
	return ERROR_NONE;
