...
#include "hi556_lce_mipi_Sensor.h"

#define PFX "hi556_lce_camera_sensor"
#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __FUNCTION__, ##args)

#define ENABLE_HI556_EEPROM
#ifdef ENABLE_HI556_EEPROM
...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.normal_video = {
		...
		.grabwindow_height = 1458,		//record different mode's height of grabwindow
		/*	 following for MIPIDataLowPwr2HighSpeedSettleDelayCount by different scenario	*/
		.mipi_data_lp2hs_settle_dc = 85,
		/*	 following for GetDefaultFramerateByScenario()	*/
		.max_framerate = 300,
...
static struct stCAM_CAL_DATAINFO_STRUCT sensor_eeprom_data ={
	.sensorID= HI556_LCE_SENSOR_ID,
	.deviceID = 2,
	.dataLength = 0x078E,
	.sensorVendorid = 0x18000000,
	.vendorByte = 1,
	.dataBuffer = NULL,
};

static struct stCAM_CAL_CHECKSUM_STRUCT lce_hi556_checksum[MAX_ITEM + 1] =
{
	{MODULE_ITEM,0x0000,0x0000,0x0007,0x0008,0x55},
	{AWB_H_ITEM,0x0009,0x0009,0x0019,0x001A,0x55},
	{AWB_M_ITEM,0x001B,0x001B,0x002B,0x002C,0x55},
	{AWB_L_ITEM,0x002D,0x002D,0x003D,0x003E,0x55},
	{LSC_ITEM,0x003F,0x003F,0x078B,0x078C,0x55},
...
static kal_uint16 hi556_lce_table_write_cmos_sensor(
...
{
#if MULTI_WRITE
hi556_lce_table_write_cmos_sensor(
...
write_cmos_sensor(0x0006, 0x0822);
write_cmos_sensor(0x0a22, 0x0000);
write_cmos_sensor(0x0a12, 0x0510);
write_cmos_sensor(0x0a14, 0x03cc);
write_cmos_sensor(0x003e, 0x0000);
write_cmos_sensor(0x0074, 0x0820);
...
//	write_cmos_sensor(0x0a00, 0x0000); // stream off
	if( currefps == 300 )
	{
		...
		write_cmos_sensor(0x003e, 0x0000);
		write_cmos_sensor(0x0074, 0x0820);
		write_cmos_sensor(0x0070, 0x0400);
		write_cmos_sensor(0x0804, 0x0200);
	...
	else
	{
		...
		write_cmos_sensor(0x0030, 0x1111);
		write_cmos_sensor(0x0032, 0x1111);
		write_cmos_sensor(0x0006, 0x1044);
		write_cmos_sensor(0x0a22, 0x0000);
		write_cmos_sensor(0x0a12, 0x0a20);
		write_cmos_sensor(0x0a14, 0x0798);
		write_cmos_sensor(0x003e, 0x0000);
		write_cmos_sensor(0x0074, 0x1042);
		write_cmos_sensor(0x0070, 0x0400);
		write_cmos_sensor(0x0804, 0x0200);
		write_cmos_sensor(0x0a04, 0x014a);
		write_cmos_sensor(0x090c, 0x0fdc);
...
hi556_lce_table_write_cmos_sensor(
		...
		write_cmos_sensor(0x0022, 0x0008);
		write_cmos_sensor(0x0028, 0x0017);
		write_cmos_sensor(0x0024, 0x011a);
		write_cmos_sensor(0x002a, 0x011f);
...
#ifdef ENABLE_HI556_EEPROM
/*Danfeng.Zhang@Camera.Driver, 2020/01/03, add for [otp bringup] */
			size = imgSensorReadEepromData(&sensor_eeprom_data, lce_hi556_checksum);
			if(size != sensor_eeprom_data.dataLength){
				pr_err("get eeprom data failed\n");
				*sensor_id = 0xFFFFFFFF;
				continue;
			} else {
				LOG_INF("get lce_hi556 eeprom data success, size=%d\n", size);
			}
#endif
			do {
				*sensor_id =return_sensor_id();
				if (*sensor_id == imgsensor_info.sensor_id) {
					...
					hardwareinfo_set_prop(HARDWARE_FRONT_CAM, "HI556");
...
UINT32 HI556_LCE_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
