...
#include "gc2385bwmipi_Sensor.h"

/************************** Modify Following Strings for Debug **************************/
#define PFX "gc2385_bw_camera_sensor"
#define LOG_1 LOG_INF("GC2385_BW, MIPI 1LANE\n")
/****************************   Modify end    *******************************************/

#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __func__, ##args)

static DEFINE_SPINLOCK(imgsensor_drv_lock);
kal_bool GC2385BWDuringTestPattern = KAL_FALSE;

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC2385_BW_SENSOR_ID,
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x17, GC2385_BW_MIRROR);
	...
	write_cmos_sensor(0x4e, GC2385_BW_BLK_Select_H);
	write_cmos_sensor(0x4f, GC2385_BW_BLK_Select_L);
	...
	write_cmos_sensor(0x92, GC2385_BW_STARTY);
	write_cmos_sensor(0x94, GC2385_BW_STARTX);
	...
	write_cmos_sensor(0x02, 0x86);//0x82
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			*sensor_id = (return_sensor_id() & 0xffef)+1;
...
static kal_uint32 open(void)
{
	...
	spin_unlock(&imgsensor_drv_lock);
	GC2385BWDuringTestPattern = KAL_FALSE;
	return ERROR_NONE;
}
...
UINT32 GC2385_BW_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
