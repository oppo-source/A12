...
#ifdef ODM_HQ_EDIT
/* Yi.Zhou@ODM.HQ.BSP.CHG.Basic 2019.11.20 for suspend soc update */
#include <linux/rtc.h>
#endif
...
#ifdef ODM_HQ_EDIT
/* Yi.Zhou@ODM.HQ.BSP.CHG.Basic 2019.11.20 for check charger resume interface */
...
#ifdef ODM_HQ_EDIT
/* Yi.Zhou@ODM.HQ.BSP.CHG.Basic 2019.11.18 make end of charge current setting work */
static int bq2560x_set_eoc_current(struct charger_device *chg_dev, u32 ua)
{
...
#ifdef ODM_HQ_EDIT
/* Yi.Zhou@ODM.HQ.BSP.CHG.Basic 2019.11.19 make bq2560x reset interface work */
int bq2560x_reset_charger(struct charger_device *chg_dev)
{
	struct bq2560x *bq = dev_get_drvdata(&chg_dev->dev);
	return 1;//bq2560x_reset_chip(bq);
}
#endif

int bq2560x_enter_hiz_mode(struct charger_device *chg_dev)
{
	struct bq2560x *bq = dev_get_drvdata(&chg_dev->dev);
...
int bq2560x_exit_hiz_mode(struct charger_device *chg_dev)
{
	struct bq2560x *bq = dev_get_drvdata(&chg_dev->dev);
...
static int bq2560x_enable_ship(struct charger_device *chg_dev)
{
  	int ret;
  	struct bq2560x *bq = dev_get_drvdata(&chg_dev->dev);
...
EXPORT_SYMBOL_GPL(bq2560x_enable_ship);
...
int bq2560x_set_rechg_voltage(struct charger_device *chg_dev, int rechg_mv)
{
	int ret;
	u8 val;
	struct bq2560x *bq = dev_get_drvdata(&chg_dev->dev);
	...
	pr_info("set recharge voltage, val = 0x%02x\n", val);
	ret = bq2560x_update_bits(bq, BQ2560X_REG_04, REG04_VRECHG_MASK, val);
	return ret;
}
#endif
...
static struct charger_ops bq2560x_chg_ops = {
	...
	.check_charging_enable = bq2560x_is_charging_enable,
	...
	.enable_ship = bq2560x_enable_ship,
	/* PE+/PE+20 */
...
/*Hanxing.Duan@ODM.HQ.BSP.CHG.Basic 2019.10.21 add bq check_charging_enable function*/
	.charger_suspend = bq2560x_enter_hiz_mode,
	.charger_unsuspend = bq2560x_exit_hiz_mode,
	.set_eoc_current = bq2560x_set_eoc_current,  /* Yi.Zhou@ODM.HQ.BSP.CHG.Basic 2019.11.18 make end of charge current setting work */
#endif /*ODM_HQ_EDIT*/
	/* ADC */
	.get_tchg_adc = NULL,
};
...
#ifdef ODM_HQ_EDIT
/* Yi.Zhou@ODM.HQ.BSP.CHG.Basic 2019.11.20 for suspend soc update and check charger resume interface */
int bq2560x_check_resume(struct charger_device *chg_dev)
{
	struct bq2560x *bq = dev_get_drvdata(&chg_dev->dev);
...
extern void oppo_chg_soc_update_when_resume(unsigned long);
...
{
	struct bq2560x *bq = dev_get_drvdata(dev);
	atomic_set(&bq->charger_suspended, 1);
