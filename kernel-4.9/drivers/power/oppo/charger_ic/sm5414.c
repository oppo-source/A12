...
unsigned int sm5414_get_suspend_status(unsigned char *val)
{
    return sm5414_read_interface((unsigned char)(SM5414_STATUS),val,
                                     (unsigned char)(SM5414_STATUS_SUSPEND_MASK),
                                     (unsigned char)(SM5414_STATUS_SUSPEND_SHIFT));
}
...
*/
	/*WT000695@ODM.BSP.Charger 2020/01/09,Modify Charger top-off current */
	sm5414_set_topoff(TOPOFF_200mA);
	sm5414_set_prechg(PRECHG_250mA);
...
static int sm5414_kick_wdt(struct charger_device *chg_dev)
{
...
static int sm5414_set_boost_ilmt(struct charger_device *chg_dev, u32 curr)
{
/*
	int ret = 0;
	struct sm5414_info *info = charger_get_data(chg_dev);

	ret = sm5414_charger_set_ichg(info, curr);
*/
	return 0;
}
...
int sm5414_reset_charger(void)
{
	sm5414_set_reset(RESET_EN);

	return 0;
}

bool sm5414_check_resume(void)
{
	unsigned int ret = 0;
	unsigned char val = 0;

	ret = sm5414_get_suspend_status(&val);
	if (ret == 0) {
		chr_info("check resume err %d\n", ret);
		return false;
	}
...
static int sm5414_enter_suspend_mode(struct charger_device *chg_dev)
{
	sm5414_set_suspen(SUSPEND_EN);
...
static int sm5414_exit_suspend_mode(struct charger_device *chg_dev)
{
	sm5414_set_suspen(SUSPEND_DIS);
...
static int sm5414_enable_ship(struct charger_device *chg_dev)
{
	int ret = 0;

	ret = sm5414_enter_suspend_mode(chg_dev);
...
static int sm5414_set_eoc_current(struct charger_device *chg_dev, u32 ua)
{
	...
	if(temp < TOPOFF_100mA) {
		curr = TOPOFF_100mA;
	} else if (temp <= TOPOFF_650mA) {
		iterm = temp / 50;
		curr = iterm * 50;
	} else {
		curr = TOPOFF_200mA;/*WT000695@ODM.BSP.Charger 2020/01/09,Modify Charger top-off current */
	}

	sm5414_set_topoff(curr);
 ...
 static const struct charger_ops sm5414_chg_ops = {
	...
	.check_charging_enable = sm5414_charger_is_enabled,
	...
	.set_boost_current_limit = sm5414_set_boost_ilmt,
	.enable_discharge = NULL,
	.enable_ship =	sm5414_enable_ship,
	/* PE+/PE+20 */
...
#ifdef ODM_HQ_EDIT
	.charger_suspend = sm5414_enter_suspend_mode,
	.charger_unsuspend = sm5414_exit_suspend_mode,
