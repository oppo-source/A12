...
#ifdef ODM_HQ_EDIT
/*Hanxing.Duan@ODM.HQ.BSP.CHG.Basic 2019.10.30 modify init SOC to RTC*/
...
static int usb_icl[] = {
	300, 500, 900, 1200, 1350, 1500, 1800, 2000, 3000,
};
...
#ifdef ODM_HQ_EDIT
/*Hanxing.Duan@ODM.HQ.BSP.CHG.Basic 2019.10.21 add for 1.8A aicl*/
	i = 6; /* 1800 */
	rc = charger_dev_set_input_current(chg, usb_icl[i] * 1000);
	msleep(90);
	chg_vol = battery_meter_get_charger_voltage();
...
/*Hanxing.Duan@ODM.HQ.BSP.CHG.Basic 2019.10.21 add bq check_charging_enable function*/
	...
	chg = g_oppo_chip->chgic_mtk.oppo_info->chg1_dev;
	rc = charger_dev_check_charging_enable(chg, &enable);
	if (rc < 0) {
		chg_debug("check  charging fail\n");
	} else {
		//chg_debug("disable charging\n");
	}
	return enable; //mt6360_check_charging_enable();
#else /*ODM_HQ_EDIT*/
	return 0; //mt6360_check_charging_enable();
#endif /*ODM_HQ_EDIT*/
...
/*Hanxing.Duan@ODM.HQ.BSP.CHG.Basic 2019.10.21 add bq check_charging_enable function*/
	...
	chg = g_oppo_chip->chgic_mtk.oppo_info->chg1_dev;
	rc = charger_dev_enter_hiz_mode(chg);
	if (rc < 0) {
...
/*Hanxing.Duan@ODM.HQ.BSP.CHG.Basic 2019.10.21 add bq check_charging_enable function*/
	...
	chg = g_oppo_chip->chgic_mtk.oppo_info->chg1_dev;
	rc = charger_dev_exit_hiz_mode(chg);
...
/* Yi.Zhou@ODM.HQ.BSP.CHG.Basic 2019.11.08 add bq2560x recharge interface */
	...
	//return bq2560x_set_rechg_voltage(g_oppo_chip->chgic_mtk.oppo_info->chg1_dev, rechg_mv);
	return 0;
#else
	//int rc = 0;
...
{
#ifdef ODM_HQ_EDIT
/* Yi.Zhou@ODM.HQ.BSP.CHG.Basic 2019.11.19 add bq2560x reset interface */
	if (!g_oppo_chip) {
		printk(KERN_ERR "[OPPO_CHG][%s]: oppo_chip not ready!\n", __func__);
		return 0;
	}
	return sm5414_reset_charger();
#else
	//int rc = 0;
...
{
#ifndef ODM_HQ_EDIT
/*Hanxing.Duan@ODM.HQ.BSP.CHG.Basic 2019.10.29 modify get vbus API*/
	...
	if (ret < 2500 || charger_type == POWER_SUPPLY_TYPE_UNKNOWN)
		vbus_status = false;
	else
		vbus_status = true;
...
/*Hanxing.Duan@ODM.HQ.BSP.CHG.Basic 2019.10.30 modify init SOC to RTC*/
	...
	new_spare3_reg = spare3_reg_valid + soc;
...
#ifndef ODM_HQ_EDIT
/*Hanxing.Duan@ODM.HQ.BSP.CHG.Basic 2019.10.21 remove for charge*/
...
{
#ifdef ODM_HQ_EDIT
/*Hanxing.Duan@ODM.HQ.BSP.CHG.Basic 2019.10.21 add bq25601 ship mode*/
	int rc = 0;
	struct charger_device *chg = NULL;
	...
	chg = g_oppo_chip->chgic_mtk.oppo_info->chg1_dev;
	rc = charger_dev_enable_ship(chg);
	if (rc < 0) {
		chg_debug("charger_dev_enable_ship fail\n");
	}
	return;
#else /*ODM_HQ_EDIT*/
	int i = 0;
...
#ifdef ODM_HQ_EDIT
	printk(KERN_ERR "[OPPO_CHG][%s]: Ingore ship mode, return!\n", __func__);
	return;
#endif
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_TYPEC_SBU_VOLTAGE:
			val->intval = 0;//oppo_get_typec_sbu_voltage();
			break;
		case POWER_SUPPLY_PROP_WATER_DETECT_FEATURE:
			val->intval = 0;//oppo_get_water_detect();
			break;
...
{
#ifndef ODM_HQ_EDIT
/*Hanxing.Duan@ODM.HQ.BSP.CHG.Basic 2019.10.29 modify otg to micro usb*/
	if (pinfo != NULL && pinfo->tcpc != NULL) {
...
/* WT000695@ODM_WT.BSP.Charger.vendor.20191225, Add MMI start/stop charging control */
#ifdef ODM_WT_EDIT
...
/* WT000695@ODM_WT.BSP.Charger.vendor.20191225, Add MMI start/stop charging control */
	...
	if (IS_ERR(oppo_chip->batt_psy) == 0) {
		ret = device_create_file(&oppo_chip->batt_psy->dev, &dev_attr_StopCharging_Test);
