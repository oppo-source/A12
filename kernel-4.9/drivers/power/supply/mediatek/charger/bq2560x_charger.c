...
static int bq2560x_get_charger_type(struct charger_device *chg_dev, u32 *charger_type)
{
	u8 val;
	int ret;
	static u32 old_type = -1;
	...
	if (old_type != *charger_type)
	{
		old_type = *charger_type;
		switch (old_type) {
		case 0:
			charger_dev_notify(chg_dev, CHARGER_DEV_NOTIFY_STOP_CHARGER);
			break;
		case 1:
		case 2:
			charger_dev_notify(chg_dev, CHARGER_DEV_NOTIFY_START_CHARGER);
			break;
		default:
			break;
		}
...
static int bq2560x_recharger(struct charger_device *chg_dev,  bool flag)
{
	u8 val;
	struct bq2560x *bq = dev_get_drvdata(&chg_dev->dev);
	if (flag)
	...
	return bq2560x_update_bits(bq, BQ2560X_REG_04, REG04_VRECHG_MASK, val);
}
...
static int bq2560x_set_ship_mode(struct charger_device *chg_dev)
{
	int ret;
	struct bq2560x *bq = dev_get_drvdata(&chg_dev->dev);
...
static int bq2560x_plug_out(struct charger_device *chg_dev)
{
	...
	ret = bq2560x_charging(chg_dev, false);
	charger_dev_notify(chg_dev, CHARGER_DEV_NOTIFY_PLUG_OUT);
	if (!ret)
...
//	.set_ta20_reset = NULL,
	...
	.enable_ship = bq2560x_set_ship_mode,
	.get_charger_type = bq2560x_get_charger_type,
	.recharger = bq2560x_recharger,
};
